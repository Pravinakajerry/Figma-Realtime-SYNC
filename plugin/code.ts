...
type CustomActionType = "UPSERT" | "DELETE" | "FULL_SYNC";
type CustomNodeType = "FRAME" | "RECTANGLE" | "TEXT" | "ELLIPSE" | "POLYGON" | "STAR" | "LINE" | "SECTION" | "GROUP" | "VECTOR" | "BOOLEAN_OPERATION";

interface SyncMessage {
    roomId?: string;
    action: CustomActionType;
    remoteNodeId?: string; // Optional for FULL_SYNC
    nodeData?: Record<string, any>; // Used for UPSERT
    nodes?: Record<string, any>[]; // Used for FULL_SYNC
}
...
const imageHashMap = new Map<string, string>(); // Remote Hash -> Local Hash

// Tracking nodes pending parent injection (no longer strictly needed for UPSERT but kept for edge safety)
...
let pendingChanges = new Map<string, SyncMessage>();
...
async function commitPendingChanges() {
    ...
    for (const msg of changesToProcess) {
        if (msg.nodeData && msg.nodeData.fills && Array.isArray(msg.nodeData.fills)) {
            const fills = msg.nodeData.fills as Paint[];
            const imageBytes: Record<string, string> = {};
            let hasImages = false;
...
function scheduleSync(msg: SyncMessage) {
...
// -----------------------------------------
function isPayloadDifferent(data1: any, data2: any): boolean {
    ...
    if (Array.isArray(data1) && Array.isArray(data2)) {
        ...
        for (let i = 0; i < data1.length; i++) {
            if (isPayloadDifferent(data1[i], data2[i])) return true;
        }
        return false;
    }
    if (data1 && typeof data1 === 'object' && data2 && typeof data2 === 'object') {
        for (const key of Object.keys(data1)) {
            if (key === 'imageBytes' || key === 'parentId' || key === 'parentIndex' || key === 'type') continue;
            if (data2[key] === undefined && data1[key] !== undefined) return true;
            if (isPayloadDifferent(data1[key], data2[key])) return true;
        }
        return false;
    }
    return true;
...
// -----------------------------------------
function extractNodePayload(node: SceneNode): Record<string, any> | null {
    if (SUPPORTED_NODES.indexOf(node.type) === -1) return null;
    ...
    for (const change of event.documentChanges) {
        ...
        if (change.type === 'CREATE' || change.type === 'PROPERTY_CHANGE') {
            ...
            if (change.type === 'PROPERTY_CHANGE' && 'children' in node && (change.properties.indexOf('x') !== -1 || change.properties.indexOf('y') !== -1)) {
                const sweepChildren = (parent: SceneNode & ChildrenMixin) => {
                    for (const child of parent.children) {
                        const childPayload = extractNodePayload(child);
                        if (childPayload) {
                            let childRemoteId = child.id;
                            for (const [remId, locId] of idMap.entries()) {
                                if (locId === child.id) { childRemoteId = remId; break; }
                            }
                            if (!idMap.has(childRemoteId)) idMap.set(childRemoteId, childRemoteId);
                            childPayload.id = childRemoteId;
                            scheduleSync({ action: 'UPSERT', remoteNodeId: childRemoteId, nodeData: childPayload });
                        }
                        if ('children' in child) sweepChildren(child as (SceneNode & ChildrenMixin));
                    }
                };
                sweepChildren(node as (SceneNode & ChildrenMixin));
            }
...
// -----------------------------------------
async function processUpsertPass1(nodeData: Record<string, any>): Promise<SceneNode | null> {
    const remoteNodeId = nodeData.id;
    if (!remoteNodeId) return null;

    let localId = idMap.get(remoteNodeId);
    let node: SceneNode | null = null;

    if (localId) {
        node = figma.getNodeById(localId) as SceneNode | null;
    }
    ...
    if (!node) {
        switch (nodeData.type) {
            ...
            case 'BOOLEAN_OPERATION': node = figma.createVector(); break;
            case 'GROUP':
    ...
    if (!hasDifference) return node;

    // UPDATE Properties
    currentlySyncingNodes.add(node.id);
    await applyProperties(node, nodeData);
    setTimeout(() => currentlySyncingNodes.delete(node!.id), 200);

    return node;
}

function processUpsertPass2(nodeData: Record<string, any>) {
    const remoteNodeId = nodeData.id;
    ...
    const node = figma.getNodeById(localId) as SceneNode;
    if (!node) return;
...
figma.ui.onmessage = async (msg) => {
    if (msg.type === 'trigger_rescan') {
        try {
            ...
            const sweep = (parent: SceneNode & ChildrenMixin | PageNode) => {
                for (const child of parent.children) {
                    ...
                    if ('children' in child) sweep(child as (SceneNode & ChildrenMixin));
                }
            };
    ...
    if (msg.type === 'remote_sync') {
        const data: SyncMessage = msg.data;
        ...
        if (data.action === 'FULL_SYNC' && data.nodes) {
            ...
            if (localNodesCount > 0 && data.nodes.length > 0) {
                ...
                for (const remoteNodeData of data.nodes) {
                    if (idMap.has(remoteNodeData.id)) continue; // Already mapped

                    // Search for a local node that is exactly the same and not already mapped
                    const sweepDiff = (parent: SceneNode & ChildrenMixin | PageNode): boolean => {
                        for (const localChild of parent.children) {
                            if (SUPPORTED_NODES.indexOf(localChild.type) === -1) continue;
                            if (matchedLocalIds.has(localChild.id)) continue;

                            // Check if this local node is already mapped to someone else
                            let isMapped = false;
                            for (const [remId, locId] of idMap.entries()) {
                                if (locId === localChild.id) { isMapped = true; break; }
                            }
                            ...
                            if ('children' in localChild) {
                                if (sweepDiff(localChild as (SceneNode & ChildrenMixin))) return true;
                            }
                        }
                        return false;
                    };
            ...
            if (data.remoteNodeId) pendingChanges.delete(data.remoteNodeId); // Clear outbound echo
            await processUpsertPass1(data.nodeData);
...
function checkPendingChildren(newRemoteParentId: string, newLocalParentId: string) {
    const parentNode = figma.getNodeById(newLocalParentId);
    if (!parentNode || !('children' in parentNode) || typeof (parentNode as any).insertChild !== 'function') return;

    const parentWithChildren = parentNode as (FrameNode | GroupNode | SectionNode | PageNode);

    for (let i = pendingChildren.length - 1; i >= 0; i--) {
        const pending = pendingChildren[i];
        if (pending.expectedParentId === newRemoteParentId) {
            const childNode = figma.getNodeById(pending.nodeId) as SceneNode | null;
            if (childNode) {
...
function placeNodeInHierarchy(node: SceneNode, parentId?: string, parentIndex?: number) {
    if (!parentId) {
    ...
    if (parentNode && 'children' in parentNode && typeof (parentNode as any).insertChild === 'function') {
        const parentWithChildren = parentNode as (FrameNode | GroupNode | SectionNode | PageNode);
        ...
        if (safeIndex > parentWithChildren.children.length) safeIndex = parentWithChildren.children.length;
...
// -----------------------------------------
async function applyProperties(node: SceneNode, data: Record<string, any>) {
    // 1. Handle Layout First
    if ('x' in data) node.x = data.x;
    if ('y' in data) node.y = data.y;
    if ('rotation' in data && 'rotation' in node) node.rotation = data.rotation;
    ...
    if ('width' in data && 'height' in data && 'resize' in node) {
        if (data.width > 0 && data.height > 0) {
            ...
            if (node.type === 'LINE') {
                try { node.resize(data.width, 0); } catch (e) { }
            } else {
    ...
    if (node.type === 'TEXT' && data.type === 'TEXT') {
        ...
        for (const prop of textProps) {
            if (prop in data && prop in textNode) {
                try {
                    (textNode as any)[prop] = data[prop];
                } catch (e) { console.warn(`Failed to set text prop ${prop}`, e); }
            }
        }
    }
    ...
    if (data.imageBytes && data.fills && Array.isArray(data.fills)) {
        for (const remoteHash of Object.keys(data.imageBytes)) {
            ...
            if (!imageHashMap.has(remoteHash)) {
                try {
                    const bytes = figma.base64Decode(base64 as string);
        ...
        for (const paint of data.fills) {
            if (paint.type === 'IMAGE' && paint.imageHash && imageHashMap.has(paint.imageHash)) {
                newFills.push({ ...paint, imageHash: imageHashMap.get(paint.imageHash) });
            } else {
                newFills.push(paint);
            }
        }
    ...
    if (data.vectorNetwork && 'setVectorNetworkAsync' in node) {
        try {
            await (node as VectorNode).setVectorNetworkAsync(data.vectorNetwork);
        } catch (e) {
